...
- **Remaining robustness follow-up:** Compatibility Mode pins `-display vnc=:0` (TCP 5900), so two Compat VMs can't run at once and a leftover QEMU squatting on 5900 blocks a spawn — needs per-VM VNC port allocation (note: the Swift viewer endpoint parser is coupled to the `vnc=:` form).

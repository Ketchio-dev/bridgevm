...
- **Application-consistent snapshots (freeze/thaw):** the daemon orchestrates guest `fs-freeze` → snapshot → `fs-thaw`, now with a structural **always-thaw** guarantee (a thaw-dispatch error can no longer leave the guest frozen) + unit tests; `backend_freeze_thaw_supported` is true for the daemon-owned path. The live e2e smoke (`application-consistent-snapshot-live-opt-in-smoke.sh`) is **WIP** — the daemon-owned guest didn't produce a GuestHello in a local run (serial empty); needs on-device debugging (seed attach via the daemon + the loopback-ext4 fsfreeze mount).

...
- **Clipboard sync + dynamic resolution — mechanism verified; desktop tooling is the user step.** The agent runs a configurable command per effect: clipboard text is piped to the command's stdin, resize passes `WIDTH HEIGHT SCALE` as args (`--clipboard-command` / `--display-resize-command`). That command execution is **unit-tested** with stub commands, and the live host→guest command transport is proven by the time-sync effects smoke — so the effect path is covered by composition. What remains is purely the user's desktop guest: pointing those commands at the real tools (`xclip`/`wl-copy`, `xrandr`) inside an Xorg/Wayland session, which only renders + verifies on a GUI desktop guest.

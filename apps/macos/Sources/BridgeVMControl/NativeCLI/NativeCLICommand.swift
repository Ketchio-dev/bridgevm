extension NativeCLIOptions.Command {
    static func parse(positionals: [String], help: Bool) throws -> Self {
        ...
        guard let verb = positionals.first, ["inspect", "readiness", "status", "stop"].contains(verb),
              positionals.count == 2 || (help && positionals.count == 1) else {
            throw NativeCLIError.invalid("Expected 'list', 'inspect ID', 'readiness ID', 'status ID' or 'stop ID'. Use --cli --help.")
        }

...
struct NativeCLIOptions: Equatable {
    enum Command: Equatable { case list, inspect(String), readiness(String), status(String), start(String), stop(String), install(String), installStatus(String), installCancel(String) }
    let command: Command
    let libraryRoot: URL
    let json: Bool
    let showHelp: Bool

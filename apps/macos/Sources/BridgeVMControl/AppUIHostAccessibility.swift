...
enum AppUIHostAccessibility {
    ...
    static func elements(in root: NSView) throws -> [any NSAccessibilityProtocol] {
        ...
        while let next = pending.popLast() {
            ...
            guard visited.count <= 10_000 else { throw AppUIHostError.refused("Owned AX tree exceeded its bound") }
    ...
    static func press(_ identifier: String, in root: NSView) throws {
        ...
        else { throw AppUIHostError.refused("Actual AX press unavailable: \(identifier)") }
    }
    ...
    static func pressButton(label: String, in root: NSView) throws {
        ...
        else { throw AppUIHostError.refused("Actual unique AX button press unavailable: \(label)") }
    }
    ...
    static func setText(_ text: String, identifier: String, in root: NSView) throws {
        ...
        else { throw AppUIHostError.refused("Actual AX text field unavailable: \(identifier)") }
        element.setAccessibilityValue(text)
        guard element.accessibilityValue() as? String == text
        else { throw AppUIHostError.refused("AX field rejected its requested value") }
    }

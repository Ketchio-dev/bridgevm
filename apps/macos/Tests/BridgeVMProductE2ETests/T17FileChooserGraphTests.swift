...
final class T17FileChooserGraphTests: XCTestCase {
    ...
    func testPublicChildAndWindowRelationshipsAreIncluded() {
        XCTAssertEqual(Set(T17FileChooserAXTree.relationships), Set([kAXChildrenAttribute, kAXWindowsAttribute]))
    }
    ...
    func testNestedSheetOpenPanelIsFoundByExactIdentityAndRole() throws {
        let nodes = try walk([0: [1], 1: [2]])
        let panel = try T17FileChooserIdentity.find(in: nodes, id: "open-panel",
            roles: [kAXWindowRole, kAXSheetRole, "AXDialog"],
            metadata: { ($0 == 2 ? "open-panel" : "unrelated", $0 == 2 ? kAXSheetRole : kAXGroupRole) }, same: ==)
        XCTAssertEqual(panel, 2)
    }

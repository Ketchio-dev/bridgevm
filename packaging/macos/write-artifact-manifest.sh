...

{
  ...
  record_helper_artifact windows_hvf_virgl_renderer "$HVF_VIRGL_RENDERER"
  record_helper_artifact windows_hvf_libepoxy "$HVF_LIBEPOXY"
  record_optional_file_artifact windows_hvf_firmware "$HVF_FIRMWARE"
  record_optional_file_artifact windows_hvf_firmware_manifest "$HVF_FIRMWARE_MANIFEST"
  record_optional_file_artifact windows_hvf_firmware_licenses "$HVF_FIRMWARE_LICENSES"
  record_optional_file_artifact windows_hvf_firmware_build_receipt "$HVF_FIRMWARE_BUILD_RECEIPT"
  record_optional_file_artifact windows_hvf_secure_boot_policy "$HVF_SECURE_BOOT_POLICY"
  if [[ "$APP_ONLY" == "0" ]]; then
    printf 'dmg.path=%s\n' "$DMG"

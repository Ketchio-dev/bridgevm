...
parse_installed_boot_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --setup-input2-actions) [[ $# -ge 2 ]] || { usage; exit 2; }; setup_input_actions_list "$2" || { echo "FAIL: --setup-input2-actions requires 1-32 supported keyboard actions; see --help" >&2; exit 2; }; SETUP_INPUT2_ACTIONS="$2"; shift 2 ;;
      ...
      --setup-input3-actions) [[ $# -ge 2 ]] || { usage; exit 2; }; setup_input_actions_list "$2" || { echo "FAIL: --setup-input3-actions requires 1-32 supported keyboard actions; see --help" >&2; exit 2; }; SETUP_INPUT3_ACTIONS="$2"; shift 2 ;;
      ...
      --pointer-input-actions) [[ $# -ge 2 ]] || { usage; exit 2; }; pointer_input_actions_list "$2" || { echo "FAIL: --pointer-input-actions requires 1-16 pointer actions (move/press/release/click/right-click/scroll) with coordinates <= 32767" >&2; exit 2; }; POINTER_INPUT_ACTIONS="$2"; shift 2 ;;

...
release_stamp = args.out / "release-unix-ns"; release_deadline = started + args.hold_ms * 1_000_000
release_code = "import pathlib,sys,time;remaining=int(sys.argv[1])-time.monotonic_ns();time.sleep(max(0,remaining)/1e9);pathlib.Path(sys.argv[2]).open('a').write(sys.argv[3]+'\\n');pathlib.Path(sys.argv[4]).write_text(str(time.time_ns()))"
release_process = subprocess.Popen([sys.executable, "-c", release_code, str(release_deadline), str(args.input_control), f"POINTER release:{args.hid_x}x{args.hid_y}", str(release_stamp)])
...
while time.monotonic_ns() <= deadline:
    ...
    time.sleep(0.005)
status = release_process.wait(); release_unix = int(release_stamp.read_text()) if status == 0 else 0

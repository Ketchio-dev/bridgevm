...
send_ok 'powershell -NoProfile -Command "Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{ CommandLine = '\''cmd /c start \"\" /b notepad.exe > C:\\BridgeVMPtr\\notepad.out 2>&1'\'' } | Out-Null; Write-Output BVNOTEPADSTARTED"'||fail 'Notepad launch failed' # redirect like the B4 probe: an inherited agent stdout handle keeps the reply pending

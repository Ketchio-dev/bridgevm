...
printf '%s\n' 'powershell -NoProfile -ExecutionPolicy Bypass -File C:\BridgeVMPtr\bvgpu-apply-host-resolution.ps1 -Width 1600 -Height 900 -LaunchNotepad' >>"$CTL"; for _ in $(seq 1 180); do grep -q '^BVNOTEPAD_LAUNCHED return=0 pid=[1-9]' "$CASE/share/bvnotepad-ready.log" 2>/dev/null&&break; sleep 1; done; tr -d '\r' <"$CASE/share/bvnotepad-ready.log" 2>/dev/null|grep -q '^BVNOTEPAD_LAUNCHED return=0 pid=[1-9]'||fail 'Notepad launch failed' # share proof, not the console reply that keeps vanishing

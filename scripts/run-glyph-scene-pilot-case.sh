...
send_ok 'powershell -NoProfile -Command "Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{ CommandLine = '\''notepad.exe'\'' } | Out-Null; Write-Output BVNOTEPADSTARTED"'||fail 'Notepad launch failed' # out-of-band: a blocking Start-Process wedges the single-threaded agent

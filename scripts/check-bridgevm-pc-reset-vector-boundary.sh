...
awk '/movz    x1, #0x1000/ {if (!called) exit 1; result=1} /bl      BridgeVmPcSecMain/ {called=1} END {exit !(called && result)}' "$ASM"
...
grep -Fq 'BootInfo->RamBase != BRIDGE_VM_PC_RAM_BASE' "$SEC/BridgeVmPcSec.c"

...
work="$(mktemp -d /tmp/bridgevm-pc-boot-modules.XXXXXX)"
...
built="$edk2/Build/BridgeVmPcBoot/RELEASE_GCC/AARCH64"
mkdir -p "$output"
...
cat >"$output/BridgeVmPcBootModules.build.json" <<EOF
{
  ...
  "exitBootServicesApplicationSha256": "${HASHES[14]}",

...
set -euo pipefail
if [[ $# -ne 12 ]]; then
  echo "usage: $0 EDK2 BUILD CORE RUNTIME VARIABLES PLATFORM CPU CPU_IO PCI_BUS PCI_HOST NVME PROBE" >&2
  exit 64
fi
edk2_root="$1"; build_root="$2"; core="$3"; runtime="$4"; variables="$5"; platform="$6"
cpu="$7"; cpu_io="$8"; pci_bus="$9"; pci_host="${10}"; nvme="${11}"; probe="${12}"
...
"$tool_root/GenFv" -o "$fv" -b 0x1000 -n 0x100 -f "$build_root/DxeCore.ffs" \
  ...
  -f "$build_root/PciHostBridge.ffs" -f "$build_root/NvmExpress.ffs" -f "$build_root/DxeProbe.ffs" \
  -g "$FFS2_GUID" --FvNameGuid "$FV_NAME_GUID"
python3 "$(dirname "$0")/check-bridgevm-pc-dxe-fv.py" "$core" "$runtime" \
  "$variables" "$platform" "$cpu" "$cpu_io" "$pci_bus" "$pci_host" "$nvme" "$probe" "$fv"

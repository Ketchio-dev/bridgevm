...
set -euo pipefail
if [[ $# -ne 7 ]]; then
  echo "usage: $0 EDK2_ROOT BUILD_ROOT DXE_CORE RUNTIME VARIABLES PLATFORM_TABLES DXE_PROBE" >&2
  exit 64
fi
edk2_root="$1"
build_root="$2"
...
readonly VARIABLE_GUID="CBD2E4D5-7068-4FF5-B462-9822B4AD8D60"
...
make_driver_ffs VariableRuntimeDxe "$VARIABLE_GUID" "$variables" \
  "$build_root/variables/VariableRuntimeDxe.depex"
...
"$tool_root/GenFv" -o "$fv" -b 0x1000 -n 0x100 \
  ...
  -f "$build_root/VariableRuntimeDxe.ffs" \
...
python3 "$(dirname "$0")/check-bridgevm-pc-dxe-fv.py" \
  "$core" "$runtime" "$variables" "$platform" "$probe" "$fv"

...
t22_marker_file_matches() { # path, pinned SHA-256
  [[ -f "$1" && ! -L "$1" ]] || return 1
  local digest; digest=$(/usr/bin/shasum -a 256 "$1") || return 1
  [[ "${digest%% *}" == "$2" ]]
}
t22_marker_log_has() {
  [[ -f "$1" ]] && tr '\r' '\n' < "$1" | grep -E "$2" > /dev/null
...
t22_marker_wait_result() { # share, nonce, action, expected marker, output, pinned script SHA
  local share=$1 nonce=$2 action=$3 expected=$4 output=$5 pinned=$6
  ...
  while (( SECONDS < deadline )); do
    if [[ -e "$done_file" || -L "$done_file" ]]; then
      ...
      if [[ -e "$result" || -L "$result" ]]; then
        ...
        actual=$(/usr/bin/shasum -a 256 "$result") || return 1
        if [[ "${actual%% *}" == "$digest" ]]; then
          ...
          t22_marker_file_matches "$share/bv-a19-t22-marker.ps1" "$pinned" || return 1
...
t22_marker_action() { # share, phase directory, control, log, action, marker, output, pinned SHA
  local share=$1 pdir=$2 ctl=$3 log=$4 action=$5 marker=$6 output=$7 pinned=$8
  local nonce command receipt result done_file pending
  nonce=$(/usr/bin/openssl rand -hex 16) || return 1
  [[ "$nonce" =~ ^[0-9a-f]{32}$ ]] || return 1
  ...
  [[ "$action" != Write || "$marker" =~ ^BV-(ORIGINAL|CLOBBERED|POSTKILL|FINAL)-[0-9a-f]{32}$ ]] || return 1
  [[ "$pinned" =~ ^[0-9a-f]{64}$ ]] || return 1
  t22_marker_file_matches "$share/bv-a19-t22-marker.ps1" "$pinned" || return 1
  command="powershell.exe -NoProfile -ExecutionPolicy Bypass -File C:\\bridgevm-share\\bv-a19-t22-marker.ps1 -Action Launch -WorkAction $action -Nonce $nonce -ExpectedSha256 $pinned"
  ...
  t22_marker_wait_result "$share" "$nonce" "$action" "$marker" "$output" "$pinned"
}
t22_boot_and_mark() { # new marker, phase name
  ...
  local ctl=$pdir/agent.ctl log=$pdir/run.log share=$pdir/share pinned_sha
  ...
  pinned_sha=$(/usr/bin/git -C "$REPO" show HEAD:scripts/win-assets/bv-a19-t22-marker.ps1 | /usr/bin/shasum -a 256) || return 1
  pinned_sha=${pinned_sha%% *}; [[ "$pinned_sha" =~ ^[0-9a-f]{64}$ ]] || return 1
  t22_marker_file_matches "$REPO/scripts/win-assets/bv-a19-t22-marker.ps1" "$pinned_sha" || return 1
  ...
  t22_marker_action "$share" "$pdir" "$ctl" "$log" Read "" "$pdir/marker-before.txt" "$pinned_sha" || return 1
  t22_marker_action "$share" "$pdir" "$ctl" "$log" Write "$marker" "$pdir/marker-after.txt" "$pinned_sha" || return 1

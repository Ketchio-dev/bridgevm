...
[[ -f "$GATE_SOURCE" && -f "$D3D11_CONFIG" ]] || fail "gate assets missing"
...
if [[ "$PRESTAGED_TITLE" != 1 ]]; then
  [[ -f "$PPSSPP_SOURCE/PPSSPPWindowsARM64.exe" ]] || fail "PPSSPP ARM64 payload missing"
fi
...
for file in bvgpu-real-title-gate.ps1 bv-ppsspp-d3d11.ini cube.iso d3d11.dll dxgi.dll; do
...
LOG=$(tr -d '\r' < "$RUN_LOG")
FPS_LINE=$(grep 'guest_fps samples=' <<< "$LOG" | tail -1)
SAMPLES=$(grep -oE 'samples=[0-9]+' <<< "$FPS_LINE" | cut -d= -f2)
P50=$(grep -oE 'p50=[0-9]+([.][0-9]+)?' <<< "$FPS_LINE" | cut -d= -f2)
MODULE_LINE=$(grep 'required_module_loaded module=d3d11.dll ' <<< "$LOG" | tail -1)
...
if [[ "${SAMPLES:-0}" -gt 0 ]] \
  ...
  A3=pass
  echo "A3 D3D11 real-title FPS: PASS (samples=$SAMPLES p50=$P50)"
else

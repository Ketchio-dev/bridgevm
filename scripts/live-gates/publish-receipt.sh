...
TIER="$1"; DIR="$2"; WORKTREE="$3"; COMMIT="$4"; [[ "$TIER" != d9-b9-real-workload ]] || exec python3 "$WORKTREE/scripts/live-gates/b9_real_workload_queue.py" publish "$DIR" "$WORKTREE" "$COMMIT"; [[ "$TIER" != d5-guest-input ]] || exec python3 "$WORKTREE/scripts/live-gates/guest_input_queue.py" publish "$DIR" "$COMMIT"; [[ "$TIER" != d4-winpe-companions ]] || exec python3 "$WORKTREE/scripts/live-gates/winpe_companion_receipt.py" publish "$DIR" "$COMMIT"; [[ "$TIER" != d1-windows-media-comparison ]] || exec python3 "$WORKTREE/scripts/live-gates/windows-media-comparison-queue.py" publish "$DIR" "$WORKTREE" "$COMMIT"

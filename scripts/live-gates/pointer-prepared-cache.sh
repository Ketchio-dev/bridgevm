...
pointer_write_result() {
  local dir="$1" image="$2" vars="$3" out="$4" package="$5" umd="$6" inf="$7"
  printf 'target=%s\nvars=%s\nimage_sha256=%s\nvars_sha256=%s\nviogpu_package_sha256=%s\ndriver_version=120.50.0.0\ninstalled_umd_sha256=%s\ninstalled_inf_sha256=%s\n' \
    "$dir/disk.raw" "$dir/vars.fd" "$image" "$vars" "$package" "$umd" "$inf" > "$out/source.env"
}

pointer_cache_matches() {
  local meta="$1" source_image="$2" source_vars="$3" package="$4" umd="$5" inf="$6"
...
if [[ ${BASH_SOURCE[0]} == "$0" ]]; then
  ...
  meta="$work/retained.env"
  {
    ...
    echo source_vars_sha256=vars; echo viogpu_package_sha256=package-a
    echo installed_umd_sha256=umd; echo installed_inf_sha256=inf
  } > "$meta"
  pointer_cache_matches "$meta" image vars package-a umd inf
  ! pointer_cache_matches "$meta" image vars package-b umd inf
  sed '/viogpu_package_sha256/d' "$meta" > "$work/legacy.env"
  ! pointer_cache_matches "$work/legacy.env" image vars package-a umd inf
  printf 'PASS: pointer prepared cache binds the exact driver package\n'
fi

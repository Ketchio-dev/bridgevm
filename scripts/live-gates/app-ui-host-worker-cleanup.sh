...
bridgevm_wait_for_app_ui_host_group() {
    ...
    if [[ "$tier" != d6-app-ui-host-v1 ]]; then
        bridgevm_wait_for_tier_group "$tier_pid" "$cancel_path" "$job_id"
        return $?
    fi
    # The launcher may await ownership for5s, then TERM3s and observe exit2s.
    ...
    if ! python3 "$worktree/scripts/live-gates/app_ui_host_cleanup.py" "$output" "$commit"; then

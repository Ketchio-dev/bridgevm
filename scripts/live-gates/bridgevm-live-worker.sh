...
run_job() {
    ...
    case "$tier" in
        ...
        t8-pointer-reliability|t12-b4-umd-diagnostic) tier_args=(--input-manifest "$manifest" --sealed-package "$dir/sealed-package" --sealed-injector-vars "$dir/sealed-injector-vars.fd" --sealed-injector-iso "$(awk -F= '$1=="sealed_injector_iso_path"{print substr($0,index($0,"=")+1)}' "$dir/job.env")") ;; t13-compatibility-observation) tier_args=(--input-manifest "$manifest" --sealed-inputs "$dir/sealed-compatibility" --sealed-package "$dir/sealed-package") ;;
    esac

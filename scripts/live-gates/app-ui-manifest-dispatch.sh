...
case "$MODE:$TIER" in
    ...
    validate:t17-windows-hvf-product-e2e) exec python3 "$HERE/windows-product-e2e-launchservices-preflight.py" --manifest "$1" ;; validate:t20-a19-native-snapshot-restore|validate:t21-a19-quota-refusal) exec python3 "$HERE/native_snapshot_restore_inputs.py" "$1" "$2" ;; validate:d9-b9-real-workload) exec python3 "$HERE/b9_real_workload_inputs.py" validate "$1" "$2" ;; seal:d9-b9-real-workload) exec python3 "$HERE/b9_real_workload_inputs.py" seal "$1" "$2" "$3" ;;

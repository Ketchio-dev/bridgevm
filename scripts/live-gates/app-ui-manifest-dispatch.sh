...
case "$MODE:$TIER" in
    ...
    validate:t17-windows-hvf-product-e2e) exec python3 "$HERE/windows-product-e2e-launchservices-preflight.py" --manifest "$1" ;; validate:t20-a19-native-snapshot-restore|validate:t21-a19-quota-refusal|validate:t22-a19-interrupted-restore) exec python3 "$HERE/native_snapshot_restore_inputs.py" "$1" "$2" ;;

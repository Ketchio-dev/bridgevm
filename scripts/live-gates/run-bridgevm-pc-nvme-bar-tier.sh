#!/usr/bin/env bash
# T11: fixed N=20 standard UEFI PciIo reads of the independent-board NVMe BAR.
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "unknown BridgeVM PC NVMe BAR tier option $1" >&2; exit 2 ;;
  esac
done
[[ -n "$OUT" ]] || { echo "BridgeVM PC NVMe BAR tier needs --out" >&2; exit 2; }
...
{
  "tier": "t11-bridgevm-pc-nvme-bar",
  "gate_id": "bridgevm-pc-standard-uefi-nvme-bar-n20",
...
for lane in $(seq 1 "$REQUIRED_LANES"); do
  ...
  if "$REPO/tests/integration/bridgevm-pc-variable-process-persistence.sh" \
     ...
     [[ "$(grep -Ec 'nvme_bar_reads=2 nvme_bar0=0x[0-9a-f]+ nvme_bar0_len=16384 nvme_cap=0x20020103ff nvme_version=0x10400 nvme_command=0x6' "$lane_dir/run.log" || true)" -eq 2 ]]; then

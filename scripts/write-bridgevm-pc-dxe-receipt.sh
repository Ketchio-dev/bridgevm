...
set -euo pipefail
if [[ $# -ne 16 ]]; then
  echo "usage: $0 OUTPUT REPO EDK2 GCC LD VECTOR_SIZE VECTOR_SHA CORE_SHA RUNTIME_SHA VARIABLE_SHA PLATFORM_SHA PCI_RECEIPT_SHA PROBE_SHA FV_SHA FD_SHA FD_SIZE" >&2; exit 64
fi
...
platform_sha="${11}" pci_receipt_sha="${12}" probe_sha="${13}" fv_sha="${14}"
artifact_sha="${15}" artifact_size="${16}"
...
printf '%s\n' '{' '  "schemaVersion": 1,' \
  '  "artifactKind": "development-only-reset-to-standard-uefi-pci-probe",' \
  ...
  "  \"pciStackReceiptSha256\": \"${pci_receipt_sha}\"," \
  "  \"dxeProbeSha256\": \"${probe_sha}\"," \
  "  \"firmwareVolumeSha256\": \"${fv_sha}\"," \
  "  \"size\": ${artifact_size}," \
  "  \"sha256\": \"${artifact_sha}\"," \
  '  "claimBoundary": "the image contains the standard UEFI PCI stack and a PCI I/O probe; enumeration, BAR operation, DMA, interrupts, boot manager, and Windows boot require separate live evidence"' \
  '}' >"$receipt"
echo "$receipt"

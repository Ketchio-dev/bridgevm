...
Default product and dashboard flows still stop at readiness. The narrow live
Fast Mode path is intentionally opt-in through the Swift `AppleVzRunner` helper
for a launch-ready `linux-kernel` + `raw` disk + NAT spec and requires
`--allow-real-vz-start`; default Fast Mode `run --spawn` remains blocked. That
live E2E can consume CPU, memory, disk, network, and Apple virtualization
resources, so it belongs in manual opt-in smoke work with known kernel/initrd/raw
disk fixtures, not in the safe Phase 0 smoke lane. See
...
guest-tools commands. Today BridgeVM treats serial sentinel evidence as the
verifier-bound guest boot progress signal; QMP state and viewer artifacts can
support console evidence without proving live boot progress by themselves. The
...
# suspend/resume are Phase-0 metadata state transitions. lifecycle-plan reports
# backend command readiness without connecting to QMP, Apple VZ, or a guest.
...
bounded host-artifact probes: the API rejects missing VMs, zero or excessive
iterations, and excessive per-sample or total probe bytes before creating the
artifact directory, and the same contract is exercised through the socket API.
...
Suspend snapshots also stay metadata-only for now. `snapshot create <vm> <name> --kind suspend` records the normal snapshot entry and writes a planned suspend image record under `metadata/suspend-images/<name>.json`. That metadata points at `suspend-images/<name>.bin`, records the image format marker, and reports whether the image file exists. BridgeVM does not serialize guest memory into that file yet. Restoring a suspend snapshot only verifies that this planned image marker exists, refreshes the image existence state, includes `suspend_image` in `metadata/last-restore.json`, and prints the suspend image path, format, readiness, and preparation timestamp. It is not memory deserialization or a live guest resume.
...
Application-consistent snapshots are still a scaffold/boundary today. `snapshot create <vm> <name> --kind application-consistent` records the snapshot entry and writes `metadata/application-consistent-snapshots/<name>.json` with guest-tools connection state, required capabilities (`fs-freeze`, `fs-thaw`), advertised capabilities, missing capabilities, readiness, backend freeze/thaw support, and planned freeze/thaw semantics. The daemon-owned `snapshot execute-application-consistent <vm> <name>` socket path can dispatch freeze/thaw scaffold messages around snapshot metadata creation when a daemon-owned backend and authenticated guest-tools stream are available. The default Linux tools path acknowledges simulated freeze/thaw protocol state, and the `--real-fsfreeze` path is an explicit opt-in backend primitive that still does not quiesce applications. Metadata-safe smoke coverage shadows `fsfreeze` with a fake executable for ordering and rollback assertions; it is not real host mount freezing, app quiescing, or proof of application consistency.

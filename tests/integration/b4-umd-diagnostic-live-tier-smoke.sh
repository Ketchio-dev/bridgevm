...
PACKAGE="$ROOT/scripts/live-gates/b4-diagnostic-package.py"
ANALYZER="$ROOT/scripts/live-gates/analyze-b4-umd-diagnostic.py"
TIER="$ROOT/scripts/live-gates/run-b4-umd-diagnostic-tier.sh"
VERIFY="$ROOT/scripts/live-gates/verify-live-sealed-input.sh"
...
if "$CLI" submit t12-b4-umd-diagnostic >/dev/null 2>&1; then fail 'T12 accepted no manifest'; fi
if "$CLI" submit t0-check --input-manifest "$manifest" >/dev/null 2>&1; then fail 'T0 accepted a manifest'; fi
job="$($CLI submit t12-b4-umd-diagnostic --input-manifest "$manifest")"
dir="$BRIDGEVM_LIVE_ROOT/queued/$job"
grep -Fqx "sealed_package_sha256=$tree_hash" "$dir/job.env"
[[ -f "$dir/input-manifest.tsv" && -d "$dir/sealed-package" ]]
"$VERIFY" t12-b4-umd-diagnostic "$dir" "$ROOT"
...
chmod 600 "$dir/sealed-package/viogpu3d.inf"; printf mutation >>"$dir/sealed-package/viogpu3d.inf"
if "$VERIFY" t12-b4-umd-diagnostic "$dir" "$ROOT"; then fail 'mutated queue package verified'; fi

...
cp "$ROOT/scripts/win-assets/"{winpeshl.ini,bvinstall.cmd,bvdiskpart.txt,unattend.xml,bvagent.ps1,bvagent-firstboot.ps1,bvagent-input.ps1,bvagent-unicode-input.cs,bvagent-key-input.cs,bvagent-task.ps1} "$TMP/assets/"
printf 'fake ISO\n' > "$TMP/windows.iso"
...
python3 "$ROOT/scripts/verify-windows-product-e2e-receipt.py" --self-test; "$ROOT/scripts/verify-product-e2e-helper-app.sh" --self-test; "$ROOT/tests/integration/windows-product-e2e-live-tier-smoke.sh"; python3 "$ROOT/tests/integration/windows-product-e2e-request-mode.py"; python3 "$ROOT/tests/integration/windows-product-e2e-signing-class.py"

...
main() {
  ...
  line="$(status_line "PARTIAL" "Windows HVF VMM" "BridgeVM now owns the QEMU-free Hypervisor.framework VMM/device stack and has preserved live proof of installed Windows 11 ARM64 reaching the desktop with four logical processors, virtio-net DHCP/DNS/HTTP/ICMP connectivity, persistent NVMe writes, framebuffer/display input, a resident virtio-console service channel, a live vTPM TIS command path, a 9/9 valid round-robin performance matrix, clean shutdown/write-back, post-exit reopen, and process-resident host pause/resume. Windows powercfg reports no available guest sleep state; durable suspend is explicitly outside v1. This metadata-only report does not rerun mutable Windows media. The current engineering gates are secure turnkey packaging/UX, PPI/Secure Boot lifecycle receipts, a finalized UMD-registered viogpu3d render candidate, and live bind/trace proof.")"
  output+="$line"$'\n'

  if bash "$ROOT/tests/integration/hvf-windows-installed-boot-vtpm-lifecycle-smoke.sh" >/dev/null \
    && [[ -x "$ROOT/apps/macos/scripts/bundle-swtpm-runtime.sh" ]] \
    && [[ -f "$ROOT/apps/macos/Sources/BridgeVMControl/HvfEngine/VTPMIdentityLifecycle.swift" ]]; then
    line="$(status_line "PASS" "Windows HVF vTPM lifecycle" "The deterministic encrypted-state launcher smoke passes. Dated live receipts prove the Windows TIS command path and PPI clear, pinned packaged swtpm 0.10.1/libtpms 0.10.2 with binary 32-byte key-FD/socket/encrypted-state cleanup, authenticated recovery export on Mac Studio, hash-matched transfer, and packaged-app same-ID restore/desktop boot on MacBook Pro M5 with 239 PCR_Read, zero malformed/backend failures, agent shutdown, PSCI system-off, and NVMe writeback (docs/windows-arm/evidence/second-mac-migration-20260723.md). BitLocker-only enable/fresh-ID contrast is blocked by Windows Home EditionID=Core, not by the vTPM lifecycle.")"
  else
  ...
  if bash "$ROOT/tests/integration/hvf-secure-boot-provisioning-smoke.sh" >/dev/null; then
    line="$(status_line "PASS" "Windows HVF Secure Boot policy" "The checked-in 3 MiB EDK2 image is pinned to an exact Secure Boot + TPM2 build and digest; the official Microsoft Windows transition policy validates four ESL payload hashes, provisions dbx/db/KEK/PK with PK last, is idempotent, and fails closed on partial/conflicting state. The dated live receipt proves Confirm-SecureBootUEFI=True, SetupMode=0, SecureBoot=1, ready TPM 2.0, a complete 62,382-byte measured-boot log, 431 PCR_Read and 82 PCR_Extend operations, and zero malformed/backend failures (docs/windows-arm/evidence/sb-guest-proof-20260723.md).")"
  else
  ...
  line="$(status_line "PASS" "Windows ARM64 viogpu3d artifact" "A dated ARM64 Venus release-candidate receipt proves a complete UMD-registered package, catalog, self-signed test certificate, render-candidate package check, host/P3 readiness, exact bound INF hash, Vulkan enumerate/draw/bench, and a passing coherent Venus trace (docs/windows-arm/evidence/viogpu3d-venus-release-candidate-20260723.md). Production EV/Partner Center signing remains external and test signing is not a distributable production signature.")"
  output+="$line"$'\n'

  line="$(status_line "EXTERNAL" "Windows ARM64 viogpu3d production signature" "The ARM64 package is submission-ready; the remaining production EV/Partner Center signature is owner/vendor infrastructure outside this repository run.")"
  output+="$line"$'\n'

  line="$(status_line "PASS" "Windows ARM64 viogpu3d live bind/trace" "The dated packaged-app receipt proves exact 120.41 bind, native ARM64 PPSSPP 1.20.4 on Venus for more than ten minutes, 115,640 parsed trace events with P3 PASS, 11,293 scanout readbacks, a 300-second fb-rate result of 13.54 FPS average, and clean PSCI/NVMe shutdown (docs/windows-arm/evidence/gpu-live-receipt-20260723.md).")"
  output+="$line"$'\n'

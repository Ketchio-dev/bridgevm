...
main() {
  ...
  line="$(status_line "PARTIAL" "Windows HVF VMM" "BridgeVM now owns the QEMU-free Hypervisor.framework VMM/device stack and has preserved live proof of installed Windows 11 ARM64 reaching the desktop with four logical processors, virtio-net DHCP/DNS/HTTP/ICMP connectivity, persistent NVMe writes, framebuffer/display input, and a resident virtio-console service channel. This metadata-only report does not rerun mutable Windows media. The current engineering gates are an agent-oracle smp=1/2/4 BOOT_TIMER matrix, clean shutdown/flush and suspend/resume proof, turnkey packaging, and live viogpu3d bind/trace proof.")"
  output+="$line"$'\n'

  line="$(status_line "PARTIAL" "Windows ARM64 viogpu3d artifact" "Preserved local CI artifacts include test-signed ARM64 INF/SYS/CAT packages for Venus and VirGL, plus a VirGL full package with five ARM64 Mesa DLLs; the repository package checker reports them injection-ready. This metadata report does not rescan those out-of-tree artifacts, and test signing is not a distributable production signature.")"
  output+="$line"$'\n'

  line="$(status_line "BLOCKED" "Windows ARM64 viogpu3d live bind/trace" "No verifier-accepted live receipt yet proves certificate trust and testsigning, pnputil install, a present DEV_1050/10F7 device with Status OK bound to the intended OEM INF, or a boot-bound coherent capset/blob/context/submit/fence trace. Package existence is no longer the blocker; this end-to-end bind and trace is.")"
  output+="$line"$'\n'

...
main() {
  ...
  if bash "$ROOT/tests/integration/hvf-windows-installed-boot-vtpm-lifecycle-smoke.sh" >/dev/null \
    ...
    line="$(status_line "PARTIAL" "Windows HVF vTPM lifecycle" "TPM TIS/PPI/TPM2-log device plumbing is deterministic-test covered, including QEMU's exact 6-byte etc/tpm/config record that activates the pinned EDK2 PPI request processor only with a concrete backend. A dated cloned Windows run reached the desktop and completed 1,032 TPM commands, including PCR, session, key-creation, and NV-public operations, with no backend or malformed-packet failures; PPI reads occurred but writes did not. The launcher smoke proves exact 32-byte key-FD delivery, AES-256-CBC encrypt-then-MAC policy, owned swtpm sockets/process cleanup, and persistent state. The packaged path pins, recursively bundles, rewrites, signs, inventories, and license-receipts swtpm/libtpms. The macOS app implements ThisDeviceOnly Keychain custody, exact-state authenticated export/restore, fresh-ID clone, and archive-before-reset receipts. Validated firmware-populated events, a live Windows PPI-action receipt, and clean-second-Mac/BitLocker recovery remain open.")"
  else
  ...
  line="$(status_line "BLOCKED" "Windows ARM64 viogpu3d render package" "No preserved package currently satisfies the repository's render-candidate gate. The pinned unsigned stage now supplies active UserModeDriverName, OpenGLDriverName, OpenGLVersion, OpenGLFlags, and InstalledDisplayDrivers registrations whose DLL names resolve through active CopyFiles entries into DirID 11; its parser-only dummy catalog is not signature evidence. Run the Windows WDK finalizer with the pre-finalization manifest and a trusted code-signing PFX, then return package-finalized and require the real render-candidate gate.")"
  output+="$line"$'\n'

  line="$(status_line "BLOCKED" "Windows ARM64 viogpu3d live bind/trace" "After a finalized UMD-registered package passes the render-candidate gate, no verifier-accepted live receipt yet proves certificate trust and testsigning, pnputil install, a present DEV_1050/10F7 device with Status OK bound to the intended OEM INF, or a boot-bound coherent capset/blob/context/submit/fence trace.")"
  output+="$line"$'\n'

...
main() {
  ...
  if bash "$ROOT/tests/integration/hvf-windows-installed-boot-vtpm-lifecycle-smoke.sh" >/dev/null \
    ...
    line="$(status_line "PARTIAL" "Windows HVF vTPM lifecycle" "TPM TIS/PPI/TPM2-log device plumbing is deterministic-test covered, including QEMU's exact 6-byte etc/tpm/config record that activates the pinned EDK2 PPI request processor only with a concrete backend. The launcher smoke proves exact 32-byte key-FD delivery, AES-256-CBC encrypt-then-MAC policy, owned swtpm sockets/process cleanup, and persistent state. The packaged path pins, recursively bundles, rewrites, signs, inventories, and license-receipts swtpm/libtpms. The macOS app implements ThisDeviceOnly Keychain custody, exact-state authenticated export/restore, fresh-ID clone, and archive-before-reset receipts. Firmware-populated events, a live Windows PPI-action receipt, clean-second-Mac/BitLocker recovery, and a live Windows receipt remain open.")"
  else

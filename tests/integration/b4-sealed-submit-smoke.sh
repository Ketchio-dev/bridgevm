...
job="$($CLI submit t8-pointer-reliability --input-manifest "$manifest")"; dir="$BRIDGEVM_LIVE_ROOT/queued/$job"
test -f "$dir/input-manifest.tsv"; grep -q '^input_manifest_sha256=[0-9a-f]\{64\}$' "$dir/job.env"

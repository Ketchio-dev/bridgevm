...
printf 'workload_profile\tshipping-core-3d-off-boot-v2\n' >> "$base"
cp "$base" "$manifest"
...
test "$(find "$BRIDGEVM_LIVE_ROOT/queued" -name input-manifest.tsv -exec awk -F '\t' '$1 == "campaign_ordinal" { ordinal=$2 } $1 == "campaign_role" { role=$2 } END { if (NR != 14) exit 1; print ordinal "=" role }' {} \; | sort | paste -sd, -)" = "1=baseline,2=candidate,3=candidate,4=baseline,5=baseline,6=candidate,7=candidate,8=baseline"
"$REPO/tests/integration/hvf-boot-performance-profile-smoke.sh" >/dev/null; python3 "$REPO/scripts/report-hvf-boot-performance-ab.py" --self-test >/dev/null; echo "HVF boot performance tier smoke: PASS"

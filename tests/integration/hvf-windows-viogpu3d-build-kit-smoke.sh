...
assert_file_contains "$OUT/run-submit-trace-build.cmd" '-DriverSysPath "%~dp0viogpu3d.sys"' "Windows submit trace runner prebuilt sys mode"
assert_file_contains "$OUT/build-viogpu3d-arm64.ps1" 'DriverSysPath = ""' "PowerShell builder prebuilt sys param"
assert_file_contains "$OUT/build-viogpu3d-arm64.ps1" "prebuilt-sys" "PowerShell builder prebuilt sys provenance"
...
python3 "$OUT/package-windows-graphics-notices.py" self-test >/dev/null || fail "copied notice gate failed"

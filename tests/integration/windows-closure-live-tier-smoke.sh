...
python3 "$RECEIPT" --self-test | grep -q PASS; for check in windows-closure-launch windows-closure-stage windows-media-comparison-manifest windows-media-comparison-runner windows-media-comparison-queue windows-media-comparison-cancel; do python3 "$ROOT/tests/integration/$check-smoke.py"; done
...
grep -Eq 'injector_boot_observed' "$TIER" && grep -Eq 'chmod 400.*disk.raw.*vars.fd' "$TIER"
...
test -f "$BRIDGEVM_LIVE_ROOT/queued/$job/input-manifest.tsv"

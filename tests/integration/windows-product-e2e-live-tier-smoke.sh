...
TIER="$ROOT/scripts/live-gates/run-windows-product-e2e-tier.sh"; MANIFEST_TOOL="$ROOT/scripts/live-gates/windows-product-e2e-manifest.py"
...
for adversary in bad-hash bad-snapshot bad-source-receipt bad-secure bad-guest bad-raw bad-audio bad-mutation; do adversary_out="$TMP/$adversary-out"; check "$adversary lane cannot produce PASS" bash -c '! "$1" --out "$2" --input-manifest "$3" --job-id "$4-fixture" >/dev/null 2>&1 && grep -q '"'"'"failure_code": "integration-failed"'"'"' "$2/receipt.json" && grep -q '"'"'"pass": false'"'"' "$2/receipt.json"' _ "$TIER" "$adversary_out" "$noresult_manifest" "$adversary"; done; first_ready_out="$TMP/first-ready-out"; check "authenticated missing READY retains installed stages as first-boot-failed" bash -c '! "$1" --out "$2" --input-manifest "$3" --job-id first-ready-missing-fixture >/dev/null 2>&1 && python3 -c '"'"'import json,sys; r=json.load(open(sys.argv[1])); assert r["failure_code"]=="first-boot-failed" and r["run_count"]==1 and r["windows_installed_passes"]==r["secure_boot_provisioned_passes"]==1 and r["first_ready_passes"]==r["passes"]==0 and r["worker_cleanup_verified"] is True and r["pass"] is False'"'"' "$2/receipt.json"' _ "$TIER" "$first_ready_out" "$noresult_manifest"; partial_out="$TMP/partial-out"; check "authenticated product failure retains its code and partial stages without passing" bash -c '! "$1" --out "$2" --input-manifest "$3" --job-id partial-fixture >/dev/null 2>&1 && python3 -c '"'"'import json,sys; r=json.load(open(sys.argv[1])); assert r["failure_code"]=="internal-error" and r["passes"]==0 and r["first_ready_passes"]==1 and r["second_shutdown_passes"]==0 and r["ui_frontend_automated"] is True and r["pass"] is False'"'"' "$2/receipt.json" && test -f "$2/private/lane-1-authenticated.json"' _ "$TIER" "$partial_out" "$noresult_manifest"; tamper_out="$TMP/request-tamper-out"; check "helper cannot mutate its sealed request" bash -c '! "$1" --out "$2" --input-manifest "$3" --job-id request-tamper-fixture >/dev/null 2>&1 && grep -q '"'"'"failure_code": "integration-failed"'"'"' "$2/receipt.json" && test ! -e "$2/private/lane-1-authenticated.json"' _ "$TIER" "$tamper_out" "$noresult_manifest"; for adversary in alias guest-alias; do check "$adversary isolation refusal and unverified cleanup remain visible" check_isolation_rejection "$TMP/$adversary-out" "$noresult_manifest" "$adversary-fixture" 1; done
check "first-READY private packet success and failure preserve receipt and cleanup" python3 "$ROOT/tests/integration/t17-private-diagnostic-packet-test.py" --tier-fixtures "$TIER" "$noresult_manifest" "$TMP" "$COMMIT"
...
check "stale T17 without a sealed worktree fences recovery" bash -c '! "$1" "$2" "$3" "$4" && test -f "$3/worker-cleanup-required" && test -d "$3/running/stale-t17" && test ! -e "$3/done/stale-t17" && test ! -e "$3/running/stale-t17/result.env"' _ "$ROOT/scripts/live-gates/recover-stale-jobs.sh" "$ROOT" "$stale_root" "$TMP/stale-work"
queue_root="$TMP/queue"; queued_id="t17-worker-fixture"; worker="$ROOT/scripts/live-gates/bridgevm-live-worker.sh"; stage_worker_fixture() {
  local dir="$queue_root/queued/$1" digest; mkdir -p "$dir"; cp "$missing_manifest" "$dir/input-manifest.tsv"; digest="$(shasum -a 256 "$dir/input-manifest.tsv" | cut -d' ' -f1)"; printf 'job_id=%s\ntier=t17-windows-hvf-product-e2e\ncommit=%s\nsubmitted_at=%s\ninput_manifest_sha256=%s\n' "$1" "$COMMIT" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$digest" > "$dir/job.env"; }
check "missing-app T17 submission is refused before ledger creation" bash -c 'mkdir -p "$1"; ! env BRIDGEVM_LIVE_ROOT="$1" "$2" submit t17-windows-hvf-product-e2e --sha "$3" --input-manifest "$4" --job-id "$5" >"$1/rejected.log" 2>&1 && grep -q "T17 app bundle is missing or unsafe" "$1/rejected.log" && test ! -e "$1/job-ledger/$5" && test ! -e "$1/queued/$5" && test ! -e "$1/running/$5" && test ! -e "$1/done/$5"' _ "$queue_root" "$ROOT/scripts/live-gates/bridgevm-live" "$COMMIT" "$missing_manifest" "$queued_id"
stage_worker_fixture "$queued_id"; check "worker-only sealed T17 fixture retains manifest handoff without a fake binary" bash -c 'test -f "$2/queued/$3/input-manifest.tsv" && test ! -e "$2/queued/$3/hvf_gic_boot_probe" && grep -Eq '"'"'^input_manifest_sha256=[0-9a-f]{64}$'"'"' "$2/queued/$3/job.env" && BRIDGEVM_REPO="$1" BRIDGEVM_LIVE_ROOT="$2" BRIDGEVM_LIVE_WORK="$4" BRIDGEVM_LIVE_MIN_FREE_GIB=0 "$5"' _ "$ROOT" "$queue_root" "$queued_id" "$TMP/worker-work" "$worker"
check "worker-only sealed worktree reaches the T17 missing-app receipt" bash -c '"$1" "$2/done/$3/receipt.public.json" --expected-commit "$4" >/dev/null && grep -q '"'"'"failure_code": "missing-app-artifact"'"'"' "$2/done/$3/receipt.json" && grep -q '"'"'"run_count": 0'"'"' "$2/done/$3/receipt.json" && grep -q '"'"'^exit_code=1$'"'"' "$2/done/$3/result.env"' _ "$VERIFY" "$queue_root" "$queued_id" "$COMMIT"
tampered_id=t17-worker-tampered; stage_worker_fixture "$tampered_id"; check "a worker-only sealed manifest mutation is staged" bash -c 'printf tampered >> "$1/queued/$2/input-manifest.tsv"' _ "$queue_root" "$tampered_id"
...
check "worker cancellation and stale recovery honor cleanup proof" python3 "$ROOT/tests/integration/t17-worker-cleanup-fence-contract.py"

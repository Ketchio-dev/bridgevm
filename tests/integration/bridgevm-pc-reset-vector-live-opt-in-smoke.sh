...
echo "$OUT" | grep -q "LIVE PROOF: BridgeVM Virtual ARM PC v1 reset vector executed at GPA zero"
echo "binary_sha256=$(shasum -a 256 "$BIN" | awk '{print $1}')"
echo "PASS: BridgeVM Virtual ARM PC v1 reset vector executed from its flash contract"

...
stages=("artifact_preflight","vm_created","source_prepared","windows_installed","secure_boot_provisioned","first_ready","keyboard_pointer","clipboard","folder_share","network","audio","first_shutdown","snapshot_restore","second_ready","second_shutdown"); nonce=r["nonce"]; prefix=nonce[:12]; clipboard=f"브리지VM T17 클립보드 왕복 v1\n{nonce}\n".encode()
...
frames=0 if "bad-audio" in r["job_id"] else 480; live="BVAGENT PONG (proactive)" if "proactive-pong" in r["job_id"] else "BVAGENT READY"; first_lines=[f"{live} FIRST",f"hda CoreAudio stats: frames_rendered={frames} drops=0 callback_errors=0","stop: PSCI SYSTEM_OFF"]; mutation_lines=[f"{live} MUTATION","stop: PSCI SYSTEM_OFF"]; final_lines=[f"{live} RESTORED","stop: PSCI SYSTEM_OFF"]
def log_body(lines):
    offsets=[]; body=b""
...
result={"schema_version":"bridgevm.windows-hvf-3d-off-product-e2e-lane.v2", "job_id":r["job_id"], "commit":r["commit"], "campaign_mode":r["campaign_mode"], "lane":r["lane"], "nonce":r["nonce"], "three_d_injection":False, "ui_frontend_automated":True, "failure_code":"none", "failure_detail": "", "cleanup_verified":True, "installer_source_path":str(source)}
...
if "first-ready-missing" in r["job_id"]: result.update({stage:False for stage in stages[5:]}); result["failure_code"]="guest-evidence-missing"; result["failure_detail"]="first boot has no BVAGENT READY evidence"

...
fn daemon_routes_compat_resume_to_supervised_handler() {
    // Without a suspend marker the supervised compat resume reports its marker
    // error, proving ResumeBackend reaches this path rather than the legacy
    // "not wired yet" response.
    let store = temp_store();
    store.create_vm(&compatibility_manifest("legacy")).unwrap();
    store
        .transition_state("legacy", VmRuntimeState::Running)

...
pub struct DeviceSurfaces {
    ...
    pub hda_audio: bool,
    /// Only a validated typed E2E launch may enable a fixed host stop request.
    pub host_diagnostic_stop: bool,
}
...
pub fn helper_env(manifest: &LaunchManifest, launch: &HelperLaunch) -> Vec<(&'static str, String)> {
    ...
    if let Some(surfaces) = &launch.surfaces {
        env.extend(surfaces::env(surfaces));
    } else {

...
pub enum EngineProductState {
    Proven,
    Compatibility,
    /// Runs real workloads end to end, but has open release-blocking gates.
    /// Tracked per criterion in `capabilities/windows-hvf.json`.
    EngineeringPreview,
}
...
impl EngineProductState {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            EngineProductState::EngineeringPreview => "ENGINEERING_PREVIEW",
        }
    }
}
...
static ENGINE_DESCRIPTORS: [VmEngineDescriptor; 3] = [
    ...
    VmEngineDescriptor {
        ...
        product_state_detail: "Runs an installed Windows 11 Arm desktop on BridgeVM's own Hypervisor.framework VMM with persistent storage, display/input, dynamic resolution, network, audio, clipboard and folder integration, TPM/Secure Boot workflows, snapshots and window Coherence verbs. 3D acceleration is excluded from General Preview and v1; release-blocking evidence remains open and known defects are disclosed below.",
    },
    VmEngineDescriptor {
...
pub fn recommend_mode(choice: &GuestChoice) -> ModeRecommendation {
    ...
    if fast {
        ...
        ModeRecommendation {
            ...
            integration: "Windows HVF Engine 1.0; not Apple VZ Fast Mode".to_string(),

...
fn format_dci3_drain_blocked(trace: Dci3DrainBlockedTrace<'_>) -> String {
    format!(
        "xHCI setup-input DCI3 drain blocked reason={reason} policy={policy} dequeue={dequeue:#x} ring_base={ring_base:#x} dcs={dcs} two_entry_rearm={two_entry_rearm} queued_reports={queued_reports} emitted_key_reports={emitted_key_reports} emitted_release_reports={emitted_release_reports}",
        reason = trace.reason,
        policy = trace.policy,
...
pub(super) fn assert_dci3_drain_blocked_trace_format_includes_parseable_state() {
    ...
    assert_eq!(
        line,
        "xHCI setup-input DCI3 drain blocked reason=no_dci3_endpoint policy=reusable_queue_drain dequeue=0x0 ring_base=0x0 dcs=false two_entry_rearm=false queued_reports=18 emitted_key_reports=1 emitted_release_reports=1"
    );
    for token in [
        "reason=no_dci3_endpoint",
        "policy=reusable_queue_drain",

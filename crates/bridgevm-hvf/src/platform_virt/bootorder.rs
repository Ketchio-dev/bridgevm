...
pub(super) const VIRTIO_BLK_PCI_BOOTORDER: &[u8] = b"/pci@i0cf8/scsi@3/disk@0,0\n\0";

const _: () = assert!(VIRTIO_BLK_BDF.0 == 0);
const _: () = assert!(VIRTIO_BLK_BDF.1 == 3);
const _: () = assert!(VIRTIO_BLK_BDF.2 == 0);

pub(super) fn virtio_blk_pci_bootorder() -> Vec<u8> {
    VIRTIO_BLK_PCI_BOOTORDER.to_vec()
}
...
mod tests {
    ...
    #[test]
    fn virtio_blk_pci_bootorder_uses_edk2_pci_scsi_path() {
        let bootorder = virtio_blk_pci_bootorder();

        assert_eq!(bootorder, VIRTIO_BLK_PCI_BOOTORDER);

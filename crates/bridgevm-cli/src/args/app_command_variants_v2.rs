include!("app_command_support.rs");
#[rustfmt::skip]
...
pub(crate) enum AppCommand {
    ...
    #[command(about = "Start or reuse the pending request already saved with this VM; ISO paths and secrets are not accepted on argv.")] Install { id: String }, #[command(about = "Read retained progress for the saved pending installation; secrets are not accepted on argv.")] InstallStatus { id: String }, #[command(about = "Request cancellation of the retained installation; secrets are not accepted on argv.")] InstallCancel { id: String }, #[command(about = "Create the saved own-HVF VM's powered-off disk and UEFI-vars snapshot.")] SnapshotCreate { id: String }, #[command(about = "Restore the saved own-HVF VM's powered-off disk and UEFI-vars snapshot.")] SnapshotRestore { id: String }, #[command(about = "Export the currently selected powered-off disk and UEFI-vars pair to an external directory.")] SnapshotExport(AppSnapshotExportArgs),
}

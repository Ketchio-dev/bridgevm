...
mod trace_metrics;
mod trace_report_print;

pub(crate) use trace_metrics::*;
pub(crate) use trace_report_print::*;
...
pub(crate) struct VirtioGpuTraceReport {
    ...
    pub(crate) command_durations_ns: Vec<u64>,
    pub(crate) command_durations_by_name: std::collections::BTreeMap<String, Vec<u64>>,
    pub(crate) submit_3d_failures: std::collections::BTreeMap<SubmitFailureKey, u64>,
...
impl VirtioGpuTraceReport {
    ...
    pub(crate) fn observe_command(&mut self, value: &serde_json::Value, line_number: usize) {
        ...
        if let Some(duration_ns) = json_u64(value, "duration_ns") {
            self.command_durations_ns.push(duration_ns);
            self.command_durations_by_name
        ...
        if response.starts_with("ERR_") {
            ...
            if name == "SUBMIT_3D" {
                let command_id = json_u64(value, "renderer_command_id")
                    .or_else(|| json_u64(value, "submit_first_command_id"));
                let key = SubmitFailureKey {
                    response: response.to_string(),
                    renderer_status: json_i64(value, "renderer_status"),
                    command_id,
                    resource_id: json_u64(value, "renderer_resource_id"),
                    resource_found: json_bool(value, "renderer_resource_found"),
                    resource_backed: json_bool(value, "renderer_resource_backed"),
                };
                *self.submit_3d_failures.entry(key).or_default() += 1;
            }
        }
    }
    ...
    pub(crate) fn command_percentile_us(&self, numerator: usize, denominator: usize) -> f64 {
        percentile_ns(&self.command_durations_ns, numerator, denominator) as f64 / 1_000.0
    }

    pub(crate) fn named_command_percentile_us(
        &self,
        name: &str,
        numerator: usize,
        denominator: usize,
        ...
        self.command_durations_by_name
            .get(name)
            .map_or(0, |values| percentile_ns(values, numerator, denominator)) as f64
            / 1_000.0
    }

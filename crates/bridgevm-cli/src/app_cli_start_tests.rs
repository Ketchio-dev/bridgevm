...
#[test]
fn start_accepts_namespace_options_before_or_after_the_exact_id() {
    for verb in ["start"] {
        for arguments in [
            vec![
                "bridgevm",
...
#[test]
fn start_forwards_one_exact_id_and_preserves_an_unusual_library_path() {
    for library in [None, Some(PathBuf::from("/owned library/with 'quotes'"))] {
        let mut expected = vec![OsString::from("--cli"), "start".into(), "개발-vm".into()];

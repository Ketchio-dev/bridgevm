...
impl AppCommand {
    pub(super) fn into_native_arguments(self) -> Vec<OsString> {
        let (verb, id) = match self {
            Self::CreateWindows(args) => return args.into_native_arguments(), Self::SnapshotExport(args) => return args.into_native_arguments(), Self::Query(query) => query.into_parts(),

...
| A19 | Disk safety | `open` | Powered-off snapshot export and restore keep a byte-exact atomic disk plus vars pair under an explicit byte quota. | Corrected: this was recorded as a gate not yet re-run, but the atomic pair did not exist. Vars used a truncating fs::write, disk export called flush() without fsync, and the copy-on-write overlay was unbounded. All three are fixed, with 39 unit tests and a live gate against the real 64 GiB image (canonical-fresh-12041-agent-20260730.raw). The gate destroyed the live disk and restored it byte-for-byte -- create, verify, restore and the post-restore read all report disk 6fef8f98... and vars d329b5b0... -- then refused a tampered snapshot without touching the live pair. Acceptance items 1, 2, 3, 4 and 6 are proven at full scale. Item 5 (a restored snapshot boots Windows and keeps a guest marker) has not been attempted, so A19 stays open. |

...
| A12 | Secure SMCCC TRNG | `open` | Guest TRNG_RND_32/64 are served only by the host OS CSPRNG; no exit-count, time, PID or constant-derived fallback exists, and provider failure returns NO_ENTROPY. | A deterministic generator derived from the vCPU exit counter is still implemented and advertised through TRNG_FEATURES. |
| A13 | PSCI 1.1 conformance | `open` | CPU_ON and AFFINITY_INFO implement the specified state table, affinity levels and error codes with atomic state transitions. | CPU_ON returns ALREADY_ON for OnPending, and AFFINITY_INFO ignores the X2 affinity level while reporting OnPending and invalid targets as OFF. |
...
| A19 | Disk safety | `open` | Powered-off snapshot export and restore keep a byte-exact atomic disk plus vars pair under an explicit byte quota. | Snapshot scope is approved for V1 but the atomic pair hash gate has not been re-run on the release runtime. |

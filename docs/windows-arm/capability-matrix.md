...
| A13 | PSCI 1.1 conformance | `open` | CPU_ON and AFFINITY_INFO implement the specified state table, affinity levels and error codes with atomic state transitions. | CPU_ON returns ALREADY_ON for OnPending, and AFFINITY_INFO ignores the X2 affinity level while reporting OnPending and invalid targets as OFF. PSCI_FEATURES no longer answers for the TRNG namespace, but the state table itself is unchanged. |

...
| Glyph blanks (caption/tab/menu) | body text fixed; synthetic probes pass while real captions fail. **Evidence-channel prerequisite resolved 2026-08-25**: the old F4 all-black 2D byte-buffer capture is not accepted as presentation; a generic CGL IOSurface snapshot now requires live geometry/storage, seed advance and a nonblack full frame, strips stride padding and retains BGRA/PPM hashes. Retained B4 active-surface bytes establish BGRA layout. The visible Notepad caption/menu/tab defect itself remains OPEN | reproduce the exact visible Notepad scene through the new active-CGL channel, then live failing-scene A/B draw-path instrumentation (context/resource/upload/barrier identity per draw) | 3/3 caption+menu+tab at 3 resolutions × 3 scales, pixel-mask verified, ±10% frame-time budget |
| Audio teardown `callback_errors=3` | **classification implemented 2026-08-25**: the sole increment was callback re-enqueue failure, and three failures matched the three in-flight AudioQueue buffers. Apple SDK contract says synchronous Stop invokes callbacks while stopping and synchronous Dispose returns after callbacks cease. A SeqCst boundary now separates `teardown_reenqueue_refusals` from `unexpected_callback_errors`, with `stop_errors` and `dispose_errors` separate | exact-SHA live playback+shutdown still required; strict ten-run summaries require frames>0, drops=0, callback_errors=0 and typed-sum consistency | 10× playback+shutdown with callback_errors=0; teardown refusals reported under their actual name |
| Driver/Secure Boot UX | **fail-closed, capability retracted 2026-08-25**. Guided install seeds Microsoft Secure Boot. Test packages are always rejected, including a contradictory TESTSIGNING-enabled report. Package-local kernel-policy booleans/hashes are unsigned self-assertions, so they now produce `kernel-policy-provenance-unverifiable` rather than authorizing injection. Missing/invalid reports also fail before source media, disks, vars or injector mutation; exact blocker and the 3D-off fallback remain. Both install and installed-disk-import factories return the same provenance blocker before source inspection, destination reservation or VM bundle creation; persisted install requests check it before filesystem/tool lookup. The old cached-injector builder, bundle clone, pending-marker writer, wrapper argument injection and marker-completion consumer are deleted. Existing user-written or historical pending markers are inert and a negative runtime test requires that neither `--placeholder-nsid1` nor the injection boot timer can appear. Developer SB-off firstboot runs read-only preflight before certificate/BCD/DriverStore mutation; kernel-policy self-claims are blocked there too | host Swift install/import no-mutation and stale-marker tests + deterministic no-control/no-surface UX policy + guest PowerShell eight-case self-test + asset/order smoke; mutations disabling host rejection or guest provenance blocker both fail. A9 historical 120.41 evidence is retained but state is OPEN | signed kernel-policy attestation verifier plus one clean-machine host flow; until then 3D injection remains unavailable and 3D-off install/import are the only product paths |
...
| Diagnostics export | **resolved 2026-08-25**: recursive manifest/log/metadata copying was removed. The product emits only a categorical VM summary, filename-free log aggregate, and six fixed structural JSON records projected onto allowlisted keys and fixed enum values under generic names. On-disk metadata redacts VM/source/output identity; raw disk/media, UEFI vars, vTPM, key, clipboard, log, manifest, path and filename fixtures are absent. Symlink/non-regular/malformed/oversized allowlist inputs abort staged export before atomic publication | focused API and daemon IPC fixed-inventory tests; local+socket `diagnostics-cli-smoke.sh` inventory parity; symlink and 16 MiB+1 refusal with zero partial files; mutations adding `secrets.json`, accepting symlinks or serializing the VM name each fail. Representative typed-store export hash `f84a1b5f02abb5fa0d24abbe4a8bde43487265a662e1f00bc44a7f7d6fb4c51d`, seven-file fixed inventory; exact-SHA T0 `20260825-052749-43311-5092`, log SHA-256 `62195c27decaa890a4c8fdc66beedac0886f1bfcd5dec94685a350a83ece209a` | **PASS**: every forbidden class (disk bytes, vars, vTPM, keys, clipboard, paths and filenames) is absent rather than merely content-redacted |
| Network breadth | **deterministic breadth resolved 2026-08-25**: fuzz input now reaches real `NatBackend::transmit`; runt/malformed, 1500-byte MTU boundary/oversize, first/non-first IPv4 fragments, missing DNS response, closed-port TCP RST and IPv6 paths are covered. Fragments are rejected because no reassembly exists; oversize frames fail closed; IPv6 has a distinct `dropped_unsupported_ipv6` counter and is explicitly unsupported rather than silently folded into `other` | 33/33 NAT tests; full HVF 859 pass (1 ignored); 9 NAT corpus seeds / 373 deterministic inputs / 0 crashes; three guard mutations fail; exact-SHA T0 `20260825-032855-64521-10149` | **PASS for deterministic breadth**; the 2026-07-24 connectivity run remains OBSERVED and is not promoted by parser tests |
...
| QMP test-socket flake | **resolved 2026-08-26**. The one-shot fake server accepted and closed while the client configured `set_read_timeout`, causing hidden first EINVAL and then retry ECONNREFUSED. PR #91 constructs the client before spawning the server and changes no production `QmpClient`. Exact `8c7f45d` Studio t10 job `20260825-182319-70701-218` reproduced the old ordering 20/20 under 24 load processes, then passed exactly 60/60 sequential full `cargo test --workspace --locked` rounds with zero failures | retained public receipt SHA-256 `2946a2d9…`, summary `43fd40a3…`; all 60 compressed logs were independently hashed and every decompressed hash matched the retained summary; `verify-qmp-stress.py` passes. Full audit: [qmp-workspace-stress-60of60-20260826.md](qmp-workspace-stress-60of60-20260826.md) | **PASS**: the fixed 20-baseline + 60-workspace acceptance gate passed without combining samples |

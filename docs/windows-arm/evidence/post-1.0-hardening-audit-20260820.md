...
| QMP test-socket flake | **cause/fix proven; full-workspace acceptance pending**. The one-shot fake server accepted and closed while the client configured `set_read_timeout`, causing hidden first EINVAL and then retry ECONNREFUSED. PR #91 constructs the client before spawning the server and changes no production `QmpClient`. A declared Studio `t10-qmp-stress` now first requires the old ordering 20/20 under 24 load processes, then exactly 60 sequential full `cargo test --workspace --locked` rounds, retaining every compressed log and raw SHA-256 | verifier mutations for 59 rounds, 19 baseline matches and changed retained log all fail; exact-SHA T0 `20260825-072323-18939-12754`, log SHA-256 `ec499af899e4ead2b31241a51f8abf5585f803f6e7023550a552c726c6c064dd` | **OPEN until t10 receipt passes 60/60** under the same 24-process load |

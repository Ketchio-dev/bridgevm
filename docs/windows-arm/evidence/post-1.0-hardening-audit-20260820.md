...
| Driver/Secure Boot UX | **fail-closed preflight implemented 2026-08-25**. Host guided install always seeds Microsoft Secure Boot, so it now accepts injected 3D only with a complete finalization report proving `kernel-policy` SYS+CAT verification; test-signed/missing/invalid/unverifiable packages are rejected in `validationError()` before the install task can build source media, create disks, seed vars or build an injector, with exact blocker and “disable 3D injection” fallback. Developer SB-off firstboot also runs a read-only guest preflight before certificate cleanup, `bcdedit /set`, cert trust or DriverStore mutation and emits exact SB/TESTSIGNING/signing/cert/PnP/problem-code state into harvested evidence | host Swift tests + guest PowerShell six-case self-test + asset/order smoke; mutations disabling host test-signing rejection or guest SB blocker both fail. Remaining proof is one clean-machine host flow | preflight report shows exact blocker; no silent BCD/SB mutation; fallback boot preserved; clean-machine receipt still required |

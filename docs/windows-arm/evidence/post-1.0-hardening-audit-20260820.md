...
| Audio teardown `callback_errors=3` | **classification implemented 2026-08-25**: the sole increment was callback re-enqueue failure, and three failures matched the three in-flight AudioQueue buffers. Apple SDK contract says synchronous Stop invokes callbacks while stopping and synchronous Dispose returns after callbacks cease. The undifferentiated counter is removed: a SeqCst boundary now separates `teardown_reenqueue_refusals` from `unexpected_callback_errors`, with `stop_errors` and `dispose_errors` separate | exact-SHA live playback+shutdown still required; verifier now fails unless unexpected/stop/dispose are all zero while preserving frames>0+drops=0 | 10× playback+shutdown with 0 unexpected callback errors; teardown refusals reported under their actual name |

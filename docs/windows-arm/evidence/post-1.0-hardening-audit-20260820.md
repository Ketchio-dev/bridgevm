...
| Driver/Secure Boot UX | **fail-closed, capability retracted 2026-08-25**. Guided install seeds Microsoft Secure Boot. Test packages are always rejected, including a contradictory TESTSIGNING-enabled report. Package-local kernel-policy booleans/hashes are unsigned self-assertions, so they now produce `kernel-policy-provenance-unverifiable` rather than authorizing injection. Missing/invalid reports also fail before source media, disks, vars or injector mutation; exact blocker and “disable 3D injection” fallback remain. The installed-disk import UI and creation API now return the same provenance blocker before source inspection, destination reservation or VM bundle creation; the old post-create path that cloned a user-writable cached injector and wrote a pending marker is removed. Developer SB-off firstboot runs read-only preflight before certificate/BCD/DriverStore mutation; kernel-policy self-claims are blocked there too | host Swift install and import no-mutation tests + guest PowerShell eight-case self-test + asset/order smoke; mutations disabling host rejection or guest provenance blocker both fail. A9 historical 120.41 evidence is retained but state is OPEN | signed kernel-policy attestation verifier plus one clean-machine host flow; until then 3D injection remains unavailable and 3D-off install/import are the only product paths |

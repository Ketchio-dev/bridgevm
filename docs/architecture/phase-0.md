...
Fast Mode and Compatibility Mode now share the same resource profile resolver before their runner-specific plans are emitted. The resolver maps the manifest's `resources.profile` to deterministic default memory, vCPU, display FPS cap, and rationale values, and Fast Mode launch planning can use the current host battery state when `memory` or `cpu` are `auto`. Runtime reapply remains a metadata-backed policy signal until live Apple VZ CPU/RAM control can apply it, but `displayd` can consume the recorded policy file for display pacing plans and the windowed Apple VZ display process exposes a Unix socket for `status`/`stop`/`policy`/`pacing`.
...
Running Fast Mode VMs can also receive a metadata-backed runtime policy signal through `bridgevm resources reapply <vm> --visibility foreground|background`, the live-control-oriented alias `bridgevm runtime-control reapply <vm> --visibility foreground|background`, or the matching daemon request. That command requires running Fast Mode runner metadata, re-reads the host battery state, applies the foreground/background policy, and records `metadata/runtime-resources.json` with the chosen memory, CPU, display FPS cap, rationale, `runtime_control_acknowledged`, and `live_apply_blockers`. The windowed Fast/VZ display path (`bridgevm display` and Show Display) records the same metadata with foreground visibility after it starts, giving display pacing consumers a policy file as soon as the visible session exists. It also records `runtime_control` metadata for the AppleVzRunner display socket (`status`, `stop`, `policy`, `pacing`). `displayd --runtime-policy-file <path>` consumes that file by applying policy visibility and numeric `display_fps_cap` to its frame pacing plan, while `bridgevm runtime-control policy <vm>` asks the live AppleVzRunner display helper to read and return the same policy file. `bridgevm runtime-control pacing <vm>` asks the helper for the display-facing visibility and FPS cap derived from that policy. During reapply, a live helper that answers that policy request sets `runtime_control_acknowledged: true`; stale or absent helpers leave it false without failing the metadata update. Until live Apple VZ CPU/RAM hot-apply exists, the record is honest: `live_applied` is `false` with `runtime-control-unavailable`, so it is a display pacing/status/policy contract rather than CPU or memory hot-plug.
...
Real Apple VZ process launch now exists at the Swift helper boundary for supported `linux-kernel` Fast Mode VMs with a `raw` primary disk and NAT networking, and it remains guarded by explicit helper configuration and live-start opt-in. Local and socket-backed planning commands still stop at metadata creation, media readiness, verification, recorded-plan download execution, and structured launch-readiness/preflight reporting; `bridgevm run <vm> --spawn`, daemon-backed Start, suspend, and resume cross into the signed `AppleVzRunner` only when `BRIDGEVM_APPLE_VZ_RUNNER` and the required opt-in are present. Without that helper boundary, the legacy dry-run fallback remains and reports that a signed runner is required. Manual live E2E can pass `--apple-vz-stop-after-seconds <N>` so the helper requests guest shutdown after startup, and the SwiftPM-built helper must be signed after build with `apps/macos/AppleVzRunner.entitlements` or an equivalent virtualization entitlement. The separate display path (`bridgevm display <vm> [--width PX --height PX]` and the app's Show Display action) launches the helper in a local GUI session with Apple VZ graphics; the demo proof paths (`scripts/run-vz-display-demo.sh --prove-window` and `--prove-proxy-crop`) capture the exact AppKit display window and, for the crop mode, preserve the app-direct RGBA framebuffer plus `displayd` crop artifacts. The crop proof path also runs `verify-vz-proxy-crop-evidence.sh` before reporting success. The headless launch and save/restore path remains separate because a graphics device disables VZ save/restore.
...
development capability set. When the guest desktop tools are present it can
enumerate visible `.desktop` applications and launch them via `gio`/`gtk-launch`,
and it can list/focus/close X11 windows via `wmctrl`; otherwise it falls back to
static or in-memory scaffold entries. These paths still remain alpha: they
exercise capability authorization, request correlation, daemon command plumbing,
and now a conservative Linux desktop tool boundary, but they do not yet provide
macOS-native window proxying or Parallels-style Coherence.
